...
- **`turbo run build --filter=@gooonzick/wizard-core`** - Build only core package
- **`turbo run test --filter=@gooonzick/wizard-react`** - Run react tests once
- **`turbo run lint:fix --filter=@gooonzick/wizard-core`** - Fix linting in core package
...
- **React package build**: `packages/react/src/index.ts` is the entry point (depends on `@gooonzick/wizard-core` via `workspace:*`)
...
- **Monorepo dependencies** - `@gooonzick/wizard-react` depends on `@gooonzick/wizard-core` via `workspace:*` in pnpm. Changes to core are immediately reflected in react tests and builds.
