...
state.stepStatuses.value; // Record<StepId, StepStatus> — status of every step
state.progress.value; // WizardProgress — computed totals, index, percentage
```
...
await actions.validateAll({ updateStatuses: true }); // Also mark invalid steps as "error"
...
await actions.cancel(); // Awaits definition.onCancel + onCancel event, then resets
...
```vue
<button @click="actions.reset()">Start over</button>
```
